...
class WOEcvAlgorithm(QgsProcessingAlgorithm):
    INPUT = 'covariates'
    STRING = 'field1'
    #STRING1 = 'field2'
    STRING2 = 'fieldlsd'
    ...
    def shortHelpString(self):
        return self.tr("This function apply Weight of Evidence to calculate susceptibility. It allows to cross-validate the analysis by k-fold cross-validation method. If you want just do fitting put k-fold equal to one")
    ...
    def load(self,parameters):
        ...
        if parameters['testN']>1:
            ...
            for i, (train, test) in enumerate(cv.split(X, y)):
                train_ind[i]=train
                test_ind[i]=test
                prob[i]=self.woe(train,test,df,nomi,parameters['txt']+'/W'+str(i)+'.txt')
                df.loc[test,'SI']=prob[i]
        elif parameters['testN']==1:
            train=np.arange(len(y))
            test=np.arange(len(y))
            prob[0]=self.woe(train,test,df,nomi,parameters['txt']+'/W.txt')
    ...
    def woe(self,train,test,frame,nomes,txt):
        df=frame.loc[train,:]
        test=frame.loc[test,:]
        nomi=nomes
        Npx1=None
